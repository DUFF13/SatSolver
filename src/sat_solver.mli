...
val random_clause : int -> clause
(** Génère une clause aléatoire contenant un nombre donné de littéraux. *)

val random_cnf : int -> int -> cnf
(** Génère une formule CNF aléatoire avec un nombre donné de variables et de clauses. *)

val print_clause : litteral list -> unit
(** Affiche une clause. *)

val print_cnf : cnf -> unit
(** Affiche une formule CNF. *)
